...
class QuoteControllerHttpTest {
    ...
    @Test
    fun `list returns paginated quotes for tenant`() {
        quoteRepo.save(Quote(tenantId = tenantId, quoteNumber = "PRES-001", customerName = "Cliente A"))
        quoteRepo.save(Quote(tenantId = tenantId, quoteNumber = "PRES-002", customerName = "Cliente B"))
        quoteRepo.save(Quote(tenantId = otherTenantId, quoteNumber = "PRES-003", customerName = "Otro"))
    ...
    fun `get returns quote by id`() {
        val quote = quoteRepo.save(Quote(tenantId = tenantId, quoteNumber = "PRES-001"))
    ...
    fun `issue from issued returns 400`() {
        val quote = quoteRepo.save(Quote(tenantId = tenantId, quoteNumber = "PRES-001", status = com.blacksmith.metalstore.quote.domain.entity.QuoteStatus.ISSUED))
    ...
    fun `cancel from accepted returns 400`() {
        val quote = quoteRepo.save(Quote(tenantId = tenantId, quoteNumber = "PRES-001", status = com.blacksmith.metalstore.quote.domain.entity.QuoteStatus.ACCEPTED))
