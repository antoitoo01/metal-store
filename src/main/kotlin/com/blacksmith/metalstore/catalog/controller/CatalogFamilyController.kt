...
import com.blacksmith.metalstore.catalog.domain.dto.response.CatalogFamilyResponse
...
class CatalogFamilyController(
    ...
    fun list(@RequestParam standard: String? = null): List<CatalogFamilyResponse> =
        (if (standard != null) repo.findByStandard(standard) else repo.findAll())
            .map { CatalogFamilyResponse.from(it) }
}
